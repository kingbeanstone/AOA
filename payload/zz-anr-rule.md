...
- **블로킹이 풀리지 않은 이유** (6절 체크리스트 결과):
  ...
  - 누가 해제해야 하는가
  - 왜 해제되지 못했는가
- **근본 원인**: 구조적 결함으로 기술. 확신도(상/중/하)를 함께 표기 (6-7).
...
교차 증거를 모은다 (한 단계에서 멈추지 말 것 — 2절 6번).

| 단계 | 무엇을 보는가 | 근거 섹션 | 세부 |
|------|-------------|----------|------|
| 1. ANR 유형 | Input / Broadcast / Service / Provider | `am_anr`, `[2]` | — |
| 2. 메인 스레드 상태 | Waiting/Blocked/Native/GC/Runnable | `[3]` | 6-1 |
| 3. 데드락 / 락 / Binder | 락 보유·순환 대기·원격 응답 지연 | `[3]` | 6-2, 6-3 |
| 4. 그래픽 파이프라인 | GPU 과점, BufferQueue 폭증, 렌더 지연 | `[4] Render`, `-k` | 6-6 |
| 5. 크로스 프로세스 의존 | system_server·원격 프로세스 대기 | `[3]`, `[5]` | 6-2 |
| 6. 일반 slow path | I/O, sleep, CPU 집약 | `[3]`, `[4]` | 6-4, 6-5 |
| 7. 근거 점수화 | 후보별 증거 강도 평가 | 위 전부 | 6-7 |
...
- **frameNumber 추이 해석:** 한 컴포넌트(특히 wallpaper)의 frameNumber 가
  ...
  단, 증가 자체가 곧 원인은 아니므로 메인 스레드 상태(6-1)·Davey!·avgGpuLoad
  와 교차로 확인한다.
...
- 생성된 `<원본>_anr_keyword_<키워드>.txt` **만** 읽고 분석한다.
  (원본 덤프 직접 읽기 금지 — 2절 절대 규칙 동일하게 적용)

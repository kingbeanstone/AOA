...
`-k` 키워드 결과는 **별도 파일이 아니라 `_anr_parsed.txt` 맨 뒤에 `[K] 키워드: <키워드>` 섹션으로 추가**된다.
키워드는 대소문자 무시, 포함 매칭, 복수 지정 가능(`-k a b c` 또는 `-k a -k b`).
노이즈 필터·압축 미적용 — 원본 줄 그대로 (라인 절단만 적용).
`-nc5` 플래그와 병용 가능.
...
- `_anr_parsed.txt` 맨 뒤의 `[K] 키워드: <키워드>` 섹션을 읽고 분석한다.
  (원본 덤프 직접 읽기 금지 — 1절 절대 규칙 동일하게 적용)
- `[K]` 섹션은 노이즈 필터·압축 없이 원본 줄 그대로 표시된다.
